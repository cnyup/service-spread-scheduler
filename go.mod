...
// Anchored to the target Kubernetes minor version (dev-design §0/§2.3):
// out-of-tree scheduler plugins must be compiled against the exact
// k8s.io/kubernetes version they will run with; there is no plugin ABI
// across minor versions. Upgrade = bump these pins + staging replaces +
// internal/spread/plugins_api.go + cmd/scheduler/main.go.
require (
	k8s.io/api v0.28.15
...
require (
	...
	k8s.io/kubelet v0.28.15 // indirect
...
// Full staging replacement: k8s.io/kubernetes v1.28.15 must be paired with
// v0.28.15 staging modules (scheduler-plugins pattern, dev-design §2.3).
replace (
	...
	k8s.io/endpointslice => k8s.io/endpointslice v0.28.15
